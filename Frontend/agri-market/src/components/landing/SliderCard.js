...
const SliderCard = ({ item }) => {
    ...
    return (
        <div className=' bg-gradient-to-t from-gray-400 to-white py-7 rounded-xl border border-gray-300 ' >
            ...
            <div className='text-center w-32 mx-auto mt-4 py-2 rounded-md border border-black cursor-pointer active:scale-90 hover:bg-gradient-to-t from-[#06f917] to-green-600 hover:text-white duration-300 ' onClick={() => quatityCheck(item.name, item.img, item.price, item.id)}>
                Add To Cart
            </div>
        </div>
    )
